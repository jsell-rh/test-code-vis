...
# ---------------------------------------------------------------------------
# Scenario: flow_paths absent — loader returns empty array
# THEN the Godot application MUST NOT crash when flow_paths is absent
# AND it treats the field as optional with a default of []
# ---------------------------------------------------------------------------

func test_flow_paths_absent_returns_empty_array() -> void:
	# Fixture does NOT contain flow_paths — field is absent.
	var data: Dictionary = {
		"nodes": [],
		"edges": [],
		"metadata": {},
	...
	var result = SceneGraphLoader.load_from_dict(data)
	_check("flow_paths" in result, "Result must contain 'flow_paths' key even when absent in input")
	_check(result["flow_paths"] is Array, "'flow_paths' must be an Array")
	_check(result["flow_paths"].size() == 0, "Missing flow_paths key should default to empty array")


func test_flow_paths_empty_list_returns_empty_array() -> void:
	# Fixture contains flow_paths: [] — explicitly empty.
	var data: Dictionary = {
		"nodes": [],
		"edges": [],
		"metadata": {},
	...
	var result = SceneGraphLoader.load_from_dict(data)
	_check("flow_paths" in result, "Result must contain 'flow_paths' key")
	_check(result["flow_paths"].size() == 0, "Empty flow_paths list should remain empty")


# ---------------------------------------------------------------------------
# Scenario: flow_paths present — loader passes through id, name, steps
# THEN the loader returns flow_paths with correct id, name, steps values
# ---------------------------------------------------------------------------

func test_flow_paths_populated_returns_all_entries() -> void:
	var data: Dictionary = {
		"nodes": [],
		"edges": [],
		"metadata": {},
		"flow_paths": [
			{"id": "order-submission", "name": "Order Submission Path", "steps": ["iam", "shared_kernel"]},
			{"id": "auth-flow", "name": "Auth Flow", "steps": ["iam"]},
		],
	}
	var result = SceneGraphLoader.load_from_dict(data)
	_check(result["flow_paths"].size() == 2, "Expected 2 flow paths, got %d" % result["flow_paths"].size())


func test_flow_path_id_is_preserved() -> void:
	var data: Dictionary = {
	...
	var result = SceneGraphLoader.load_from_dict(data)
	var fp: Dictionary = result["flow_paths"][0]
	_check("id" in fp, "Flow path must have 'id' field")
	_check(fp["id"] == "order-submission", "flow_paths[0].id should be 'order-submission'")


func test_flow_path_name_is_preserved() -> void:
	var data: Dictionary = {
		"nodes": [], "edges": [], "metadata": {},
		"flow_paths": [
			{"id": "order-submission", "name": "Order Submission Path", "steps": ["iam", "shared_kernel"]},
	...
	var result = SceneGraphLoader.load_from_dict(data)
	var fp: Dictionary = result["flow_paths"][0]
	_check("name" in fp, "Flow path must have 'name' field")
	_check(fp["name"] == "Order Submission Path", "flow_paths[0].name should be 'Order Submission Path'")


func test_flow_path_steps_are_preserved_in_order() -> void:
	var data: Dictionary = {
		"nodes": [], "edges": [], "metadata": {},
		"flow_paths": [
			{"id": "order-submission", "name": "Order Submission Path", "steps": ["iam", "shared_kernel"]},
	...
	var result = SceneGraphLoader.load_from_dict(data)
	var fp: Dictionary = result["flow_paths"][0]
	_check("steps" in fp, "Flow path must have 'steps' field")
	_check(fp["steps"] is Array, "flow_paths[0].steps must be an Array")
	_check(fp["steps"].size() == 2, "Expected 2 steps, got %d" % fp["steps"].size())
	_check(fp["steps"][0] == "iam", "steps[0] should be 'iam' (entry point)")
	_check(fp["steps"][1] == "shared_kernel", "steps[1] should be 'shared_kernel' (terminus)")
