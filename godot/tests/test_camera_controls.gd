...
func test_orbit_horizontal_drag_changes_phi() -> bool:
	# drag right → delta.x = +50 → _phi -= delta.x * orbit_speed → _phi decreases → azimuth changes ✓
	var cam = CameraScript.new()
...
func test_orbit_vertical_drag_changes_theta() -> bool:
	# drag up → screen Y decreases → delta.y = -20 → _theta -= delta.y * orbit_speed → _theta decreases → camera tilts toward zenith ✓
	var cam = CameraScript.new()
