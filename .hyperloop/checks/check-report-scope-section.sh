...
if [ ! -f "$REPORT" ]; then
  # Attempt to recover from git history, skipping commits where the file was empty.
  # Orchestrator cleanup commits wipe worker-result.yaml content without removing the
  # file from git tracking — a plain `head -1` would recover the empty cleanup version
  # and produce a false FAIL that propagates as a RACF into the next cycle.
  # Instead, iterate through all commits touching the file and take the first with
  # non-empty content.
  while IFS= read -r sha; do
    [[ -z "$sha" ]] && continue
    REPORT_CONTENT=$(git show "${sha}:${REPORT}" 2>/dev/null)
    if [ -n "$REPORT_CONTENT" ]; then
      echo "NOTE: $REPORT absent from working tree; recovering from commit ${sha:0:7}."
      RECOVERED=1
      TMP=$(mktemp)
      printf '%s\n' "$REPORT_CONTENT" > "$TMP"
      REPORT="$TMP"
  ...
  done < <(git log --format="%H" -- "$REPORT" 2>/dev/null)

  if [ "$RECOVERED" -eq 0 ]; then
    echo "FAIL: $REPORT not found and no prior git commit contains non-empty content."
    echo "      Draft your report before running this check."
    exit 1
  fi
fi
