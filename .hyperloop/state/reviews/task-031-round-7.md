...
50 checks verified (one more than the prior attempt's 49, indicating a new check was added to main).
...
No commits were made on this branch beyond what was present before this attempt.
No implementation code was written. No files were staged.

### Classification

SCOPE-PROHIBITION FAIL — Retry is FORBIDDEN.

This is at minimum the THIRD prohibited-spec assignment for this spec
(task-028 → task-031 → task-031 re-attempt per Findings).
...
5. The check-retry-not-scope-prohibited.sh script now detects this at EXIT 1
   correctly when given the task ID. The no-arg exit-0-SKIP behavior should be
   verified on the current main (50 checks present vs 49 at prior attempt).