...
This is attempt 4 of task-028. The mandatory Sync Point 1 and scope check were
...
`check-checks-in-sync.sh` exited 0:
  OK: All check scripts from main are present and content-identical in working tree (47 checked).

(Note: 47 scripts on this attempt vs. 46 on attempt 3 vs. 45 on attempt 2 vs. 44
on attempt 1 — one additional check script has been added to main between each run,
confirming that Sync Point 1 is necessary and functioning correctly.)
...
This is the fourth consecutive attempt with the same outcome.
...
on every attempt. This is attempt 4; there is no new information available to an
