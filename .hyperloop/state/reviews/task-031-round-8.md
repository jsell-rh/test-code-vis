...
Checks synced from origin/main before any action:
  ...
  → OK: All check scripts from main are present and content-identical in working tree (51 checked).

51 checks verified (one more than the prior attempt's 50, indicating a new check was added to main).
...
This is at minimum the FOURTH prohibited-spec assignment attempt for this spec
(task-028 → task-031 attempt 1 → task-031 attempt 2 (prior Findings) →
task-031 attempt 3 (this attempt)).
...
3. This spec has been assigned multiple times in violation of scope. Investigate
   and permanently close the gap that allows prohibited specs back into the
   candidate pool.
