...
Extend the JSON schema and the Python extractor so that the full body text of each
spec requirement section is stored in the scene graph, making the spec's intent
directly available for downstream inspection alongside the realized codebase structure.
