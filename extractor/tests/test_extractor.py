...
class TestDependencyExtraction:
    ...
    def test_cross_context_edge_has_weight(self, src: Path) -> None:
        """Every cross_context edge carries a weight field (import count >= 1).
    ...
    def test_internal_edge_has_weight(self, src: Path) -> None:
        """Every internal edge carries a weight field (import count >= 1).
    ...
    def test_cross_context_edge_weight_accumulates_multiple_imports(
        self, tmp_path: Path
    ) -> None:
        """cross_context edge weight >= 2 when module A has 2 import statements from module B.
        ...
        # Bounded context: svc
        svc = tmp_path / "svc"
        (svc / "handlers").mkdir(parents=True)
        for d in [svc, svc / "handlers"]:
            (d / "__init__.py").write_text("")

        # Bounded context: shared
        shared = tmp_path / "shared"
        (shared / "utils").mkdir(parents=True)
        for d in [shared, shared / "utils"]:
            (d / "__init__.py").write_text("")
        (shared / "utils" / "helpers.py").write_text(
            "def helper_a(): pass\ndef helper_b(): pass\n"
        )

        # svc.handlers imports 2 symbols from shared → 2 import statements
        (svc / "handlers" / "service.py").write_text(
            "from shared.utils import helper_a\n"
            "from shared.utils import helper_b\n"
            "def handle(): pass\n"
        )
        ...
        # The svc→shared cross_context edge should have weight >= 2
        svc_shared_edges = [
            e for e in cc_edges if e["source"] == "svc" and e["target"] == "shared"
        ]
        assert svc_shared_edges, "Expected a svc→shared cross_context edge"
        edge = svc_shared_edges[0]
        assert edge.get("weight", 0) >= 2, (
            f"svc→shared cross_context edge must have weight >= 2 (two import "
            f"statements); got weight={edge.get('weight')}"
        )

    def test_internal_edge_weight_accumulates_multiple_imports(
        self, tmp_path: Path
    ) -> None:
        """internal edge weight >= 2 when module A has 2 import statements from module B.

        Spec: 'each edge carries the import count (number of individual
        import statements between the pair).'
        """
        # Bounded context: orders with two modules
        orders = tmp_path / "orders"
        domain = orders / "domain"
        application = orders / "application"
        for d in [orders, domain, application]:
            d.mkdir(parents=True)
            (d / "__init__.py").write_text("")

        (domain / "models.py").write_text("class Order: pass\nclass LineItem: pass\n")
        # application imports 2 distinct things from domain → 2 import statements
        (application / "services.py").write_text(
            "from orders.domain import Order\n"
            "from orders.domain import LineItem\n"
            "def process(o: Order, li: LineItem): pass\n"
        )
        ...
        # The orders.application→orders.domain internal edge should have weight >= 2
        app_domain_edges = [
            e
            for e in internal_edges
            if e["source"] == "orders.application" and e["target"] == "orders.domain"
        ]
        assert app_domain_edges, (
            "Expected an orders.application→orders.domain internal edge"
        )
        edge = app_domain_edges[0]
        assert edge.get("weight", 0) >= 2, (
            f"orders.application→orders.domain internal edge must have weight >= 2 "
            f"(two import statements); got weight={edge.get('weight')}"
        )
