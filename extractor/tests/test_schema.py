...
class TestValidateSceneGraph:
    ...
    def test_flow_paths_empty_list_is_valid(self) -> None:
        """flow_paths: [] is a valid optional field."""
        graph = make_scene_graph()
        graph["flow_paths"] = []  # type: ignore[typeddict-unknown-key]
        validate_scene_graph(graph)  # must not raise

    def test_graph_without_flow_paths_is_valid(self) -> None:
        """flow_paths absence is valid — field is optional."""
        graph = make_scene_graph()
        assert "flow_paths" not in graph
        validate_scene_graph(graph)  # must not raise

    def test_valid_flow_path_passes_validation(self) -> None:
        """A graph with a well-formed flow path passes validation."""
        graph = make_scene_graph()
        graph["flow_paths"] = [  # type: ignore[typeddict-unknown-key]
            {
                "id": "order-submission",
                "name": "Order Submission Path",
                "steps": ["iam", "shared_kernel"],
            }
        ]
        validate_scene_graph(graph)  # must not raise

    def test_flow_path_missing_id_raises(self) -> None:
        graph = make_scene_graph()
        graph["flow_paths"] = [  # type: ignore[typeddict-unknown-key]
            {"name": "Order Submission Path", "steps": ["iam"]}
        ]
        try:
            validate_scene_graph(graph)
            assert False, "Expected ValueError"
    ...
    def test_flow_path_missing_name_raises(self) -> None:
        graph = make_scene_graph()
        graph["flow_paths"] = [  # type: ignore[typeddict-unknown-key]
            {"id": "order-submission", "steps": ["iam"]}
        ]
        try:
            validate_scene_graph(graph)
            assert False, "Expected ValueError"
    ...
    def test_flow_path_missing_steps_raises(self) -> None:
        graph = make_scene_graph()
        graph["flow_paths"] = [  # type: ignore[typeddict-unknown-key]
            {"id": "order-submission", "name": "Order Submission Path"}
        ]
        try:
            validate_scene_graph(graph)
            assert False, "Expected ValueError"
    ...
    def test_flow_path_non_dict_entry_raises(self) -> None:
        graph = make_scene_graph()
        graph["flow_paths"] = ["not-a-dict"]  # type: ignore[typeddict-unknown-key]
        try:
            validate_scene_graph(graph)
            assert False, "Expected ValueError"
        except ValueError as e:
...
# ---------------------------------------------------------------------------
# Requirement: FlowPath TypedDict
# ---------------------------------------------------------------------------


def make_flow_path() -> FlowPath:
    """Return a minimal valid FlowPath."""
    return {
        "id": "order-submission",
        "name": "Order Submission Path",
        "steps": ["iam", "shared_kernel"],
...
class TestFlowPathSchema:
    """FlowPath TypedDict MUST have id, name, and steps fields."""

    def test_flow_path_has_id(self) -> None:
        fp = make_flow_path()
        assert fp["id"] == "order-submission"

    def test_flow_path_has_name(self) -> None:
        fp = make_flow_path()
        assert fp["name"] == "Order Submission Path"

    def test_flow_path_has_steps(self) -> None:
        fp = make_flow_path()
        assert fp["steps"] == ["iam", "shared_kernel"]

    def test_flow_path_id_is_str(self) -> None:
        fp = make_flow_path()
        assert isinstance(fp["id"], str)

    def test_flow_path_name_is_str(self) -> None:
        fp = make_flow_path()
        assert isinstance(fp["name"], str)

    def test_flow_path_steps_is_list(self) -> None:
        fp = make_flow_path()
        assert isinstance(fp["steps"], list)

    def test_flow_path_steps_are_strings(self) -> None:
        fp = make_flow_path()
        for step in fp["steps"]:
            assert isinstance(step, str)

    def test_flow_path_steps_ordered_entry_to_terminus(self) -> None:
        """Steps represent an ordered traversal from entry to terminus."""
        fp = make_flow_path()
        # Entry is first, terminus is last.
        assert fp["steps"][0] == "iam"
        assert fp["steps"][-1] == "shared_kernel"

    def test_flow_path_steps_can_be_empty(self) -> None:
        fp: FlowPath = {"id": "empty-path", "name": "Empty Path", "steps": []}
        assert fp["steps"] == []

    def test_flow_path_is_json_serialisable(self) -> None:
        import json
        ...
        assert restored["steps"] == fp["steps"]

    def test_scene_graph_flow_paths_field_is_optional(self) -> None:
        """SceneGraph.flow_paths is NotRequired — absence is valid."""
        graph = make_scene_graph()
        assert "flow_paths" not in graph  # not present by default

    def test_scene_graph_with_flow_paths_is_valid(self) -> None:
        graph = make_scene_graph()
        graph["flow_paths"] = [make_flow_path()]  # type: ignore[typeddict-unknown-key]
        assert len(graph["flow_paths"]) == 1
        assert graph["flow_paths"][0]["id"] == "order-submission"
