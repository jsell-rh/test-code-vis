...
# ---------------------------------------------------------------------------
# Requirement: Node Metrics Validation (task-119)
#
# The `metrics` field is optional on all node types.  When present:
#   - it MUST be a dict
#   - `metrics.loc`, when present, MUST be a non-negative integer
# ---------------------------------------------------------------------------


class TestNodeMetricsValidation:
    """validate_scene_graph enforces the metrics object contract."""

    def test_valid_graph_without_metrics_passes(self) -> None:
        """Absent metrics field is valid — metrics is optional."""
        graph = make_scene_graph()
        # make_scene_graph returns nodes without a metrics key
        for node in graph["nodes"]:
            assert "metrics" not in node
        validate_scene_graph(graph)  # must not raise

    def test_valid_graph_with_metrics_loc_passes(self) -> None:
        """metrics with a non-negative loc passes validation."""
        graph = make_scene_graph()
        graph["nodes"][0]["metrics"] = {"loc": 3200}  # type: ignore[typeddict-unknown-key]
        validate_scene_graph(graph)  # must not raise

    def test_metrics_zero_loc_passes(self) -> None:
        """metrics.loc == 0 is a valid non-negative integer."""
        graph = make_scene_graph()
        graph["nodes"][0]["metrics"] = {"loc": 0}  # type: ignore[typeddict-unknown-key]
        validate_scene_graph(graph)  # must not raise

    def test_metrics_not_a_dict_raises(self) -> None:
        """metrics must be a dict; a non-dict value raises ValueError."""
        graph = make_scene_graph()
        graph["nodes"][0]["metrics"] = 42  # type: ignore[typeddict-unknown-key]
        try:
            validate_scene_graph(graph)
            assert False, "Expected ValueError"
        except ValueError as e:
    ...
    def test_metrics_as_list_raises(self) -> None:
        """metrics must be a dict; a list raises ValueError."""
        graph = make_scene_graph()
        graph["nodes"][0]["metrics"] = [{"loc": 100}]  # type: ignore[typeddict-unknown-key]
        try:
            validate_scene_graph(graph)
            assert False, "Expected ValueError"
        except ValueError as e:
    ...
    def test_metrics_loc_negative_raises(self) -> None:
        """metrics.loc must be >= 0; a negative value raises ValueError."""
        graph = make_scene_graph()
        graph["nodes"][0]["metrics"] = {"loc": -1}  # type: ignore[typeddict-unknown-key]
        try:
            validate_scene_graph(graph)
            assert False, "Expected ValueError"
        except ValueError as e:
    ...
    def test_metrics_loc_float_raises(self) -> None:
        """metrics.loc must be an integer; a float raises ValueError."""
        graph = make_scene_graph()
        graph["nodes"][0]["metrics"] = {"loc": 100.5}  # type: ignore[typeddict-unknown-key]
        try:
            validate_scene_graph(graph)
            assert False, "Expected ValueError"
        except ValueError as e:
    ...
    def test_metrics_loc_bool_raises(self) -> None:
        """metrics.loc must not be a bool (bool is a subtype of int in Python)."""
        graph = make_scene_graph()
        graph["nodes"][0]["metrics"] = {"loc": True}  # type: ignore[typeddict-unknown-key]
        try:
            validate_scene_graph(graph)
            assert False, "Expected ValueError"
        except ValueError as e:
    ...
    def test_metrics_without_loc_passes(self) -> None:
        """metrics object may omit loc — loc is only validated when present."""
        graph = make_scene_graph()
        graph["nodes"][0]["metrics"] = {}  # type: ignore[typeddict-unknown-key]
        validate_scene_graph(graph)  # must not raise

    def test_metrics_on_module_node_passes(self) -> None:
        """metrics is valid on module nodes too, not just bounded_context."""
        graph = make_scene_graph()
        graph["nodes"][1]["metrics"] = {"loc": 450}  # type: ignore[typeddict-unknown-key]
        validate_scene_graph(graph)  # must not raise

    def test_metrics_loc_distinguishable_from_size(self) -> None:
        """metrics.loc is the raw integer count; size is the normalised float.

        They serve different purposes: loc for display ("LOC: 3,200"),
        size for Godot's MeshInstance3D scale.  Both may coexist on the same node.
        """
        node = make_bounded_context_node()
        node["metrics"] = {"loc": 3200}  # type: ignore[typeddict-unknown-key]
        # size is already present on the node from the fixture
        assert isinstance(node["size"], float)
        assert isinstance(node["metrics"]["loc"], int)
        assert node["metrics"]["loc"] != node["size"]
