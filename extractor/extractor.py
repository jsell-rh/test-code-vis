...
def build_dependency_edges(src_path: Path, all_nodes: list[Node]) -> list[Edge]:
    ...
    ``graph → shared_kernel``) with ``type="cross_context"`` and a ``weight``
    equal to the count of unique module-level imports between the two contexts.
    For each BC-pair an additional ``type="aggregate"`` edge is also emitted
    carrying that same total import count as its ``weight`` (used for
    far-distance rendering).  Internal edges are created at the module level
    (e.g. ``iam.application → iam.domain``) and also carry a ``weight``
    equal to the count of unique imports between the two modules.

    Duplicate edges are deduplicated; weights accumulate across all
    module-level scans.
    """
...
# (visual-primitives.spec.md §Requirement: Data Flow Spine Extraction)
# ---------------------------------------------------------------------------
...
def _trace_parameter_spine(
    ...
    THEN the spine from parameter ``input`` through each operation to the
    return value is emitted, AND each step in the spine references the
    intermediate function or expression.
    ...
    for stmt in ast.walk(func_node):
        ...
        if isinstance(stmt, ast.Assign):
            ...
            if rhs_names & tainted:
                # Determine assignment target (simple name).
                lhs_name = "unknown"
                ...
                # Interprocedural: check if the RHS is a call to a known function.
                # Spec: "the spine includes: A's x → B's parameter → B's return → A's y"
                # We only go ONE call level deep — no recursion into callee's callee.
                if isinstance(stmt.value, ast.Call):
                    ...
                    if callee_name:
                        ...
                        if callee_module and callee_module != source_module_id:
        ...
        # Return: return <expr using a tainted name>
...
def extract_data_flow_spines(src_path: Path, all_nodes: list[Node]) -> list[dict]:
    ...
    flows through the function body to the return value.  Also records
    one-call-deep interprocedural flow when a parameter is passed to a known
    callee whose return value is assigned to a local variable.

    The result is stored as ``node['data_flow_spines']`` (a list of spine dicts)
    on each module node that has at least one traceable spine.  The function
    also returns the complete flat list for callers that want a single collection.
    ...
    THEN the spine from parameter *input* through each operation to the return
         value is emitted, AND each step in the spine references the
         intermediate function or expression.
...
def build_scene_graph(src_path: Path) -> SceneGraph:
    ...
    #     Annotates module nodes with data_flow_spines list in-place.
    #     Spec: visual-primitives.spec.md §Requirement: Data Flow Spine Extraction.
    extract_data_flow_spines(src_path, nodes)

    # 14. Assemble metadata.
    metadata: Metadata = {
