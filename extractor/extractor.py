...
# (visual-primitives.spec.md §Data Flow Spine Extraction)
# ---------------------------------------------------------------------------


class DataFlowStep(dict):
    """A single step in a data flow spine.

    Keys:
        source_ref: str   — human-readable description of where the value originates,
                            e.g. 'param:input', 'call:validate', 'return'.
        target_ref: str   — where the value flows to, e.g. 'local:result', 'return'.
        step_type: str    — 'param_use', 'call', 'assign', or 'return'.
    """


class DataFlowSpine(dict):
    """A complete intraprocedural data flow chain for one function.

    Keys:
        module_id: str         — ID of the module containing the function.
        function_name: str     — name of the function being analysed.
        parameter: str         — name of the parameter being traced.
        steps: list[dict]      — ordered list of DataFlowStep dicts.
        interprocedural: list  — one-call-deep interprocedural extensions, each
                                 a dict with keys: call_name, callee_param,
                                 callee_return, assign_to.
    """


def _trace_parameter_spine(
    func_node: ast.FunctionDef | ast.AsyncFunctionDef,
    param_name: str,
    fn_to_module: dict[str, str],
    source_module_id: str,
) -> dict:
    """Trace how *param_name* flows through *func_node*'s body.

    Produces an intraprocedural spine from the parameter through each
    assignment/call site to the return value.  Also records one-call-deep
    interprocedural flow when the parameter is passed to a known callee.
    ...
    THEN the spine from parameter *input* through each operation to the return
         value is emitted, AND each step in the spine references the intermediate
         function or expression.

    Args:
        func_node:        AST function definition node.
        param_name:       The parameter name to trace.
        fn_to_module:     Map from function name → module ID (for interprocedural).
        source_module_id: Module ID where func_node lives.

    Returns:
        A dict with keys 'function_name', 'parameter', 'steps', 'interprocedural'.
    """
    steps: list[dict] = []
    interprocedural: list[dict] = []

    # Step 1: parameter → entry point.
    # param:X is the canonical source_ref for a function parameter.
    steps.append(
        {
            "source_ref": f"param:{param_name}",
            "target_ref": "body",
            "step_type": "param_use",
        }
    )

    # Taint set: names whose current value is derived from param_name.
    # Starts with the parameter itself; grows as we see assignments like
    # ``result = f(param_name)`` — result is then also tainted.
    # This enables us to follow the flow: input → result → return.
    tainted: set[str] = {param_name}

    # Walk the function body (not nested functions) to find:
    # - assignments that reference a tainted name on the RHS
    # - calls that pass a tainted name as an argument
    # - return statements that return a tainted name
    for stmt in ast.walk(func_node):
        # Skip nested function defs to stay intraprocedural.
        if stmt is not func_node and isinstance(
            stmt, (ast.FunctionDef, ast.AsyncFunctionDef)
        ):
            continue

        # Assignment: y = f(tainted) or y = tainted  (propagates taint to y)
        if isinstance(stmt, ast.Assign):
            # Check if any value node references a tainted name.
            rhs_names = {n.id for n in ast.walk(stmt.value) if isinstance(n, ast.Name)}
            if rhs_names & tainted:
                # Determine what we're assigning to (simple name or subscript).
                lhs_name = "unknown"
                ...
                steps.append(
                    {
                        "source_ref": f"param:{param_name}",
                        "target_ref": f"local:{lhs_name}",
                        "step_type": "assign",
                    }
                )
                # Propagate taint to the LHS variable so downstream uses
                # (e.g. ``return lhs_name``) are also captured.
                if lhs_name != "unknown":
                    tainted.add(lhs_name)

                # Check if RHS is a call that takes a tainted arg — interprocedural.
                if isinstance(stmt.value, ast.Call):
                    callee_name = ""
                    if isinstance(stmt.value.func, ast.Name):
                        callee_name = stmt.value.func.id
                    elif isinstance(stmt.value.func, ast.Attribute):
                        callee_name = stmt.value.func.attr
                    # Verify a tainted name is actually passed as an arg.
                    arg_names = {
                        n.id
                    ...
                    if callee_name and arg_names & tainted:
                        callee_module = fn_to_module.get(callee_name)
                        if (
                            callee_module is not None
                            and callee_module != source_module_id
                        ):
                            # One-call-deep interprocedural spine.
                            # spec: "A's x → B's parameter → B's return → A's y"
                            interprocedural.append(
                                {
                                    "call_name": callee_name,
                                    "callee_module": callee_module,
                                    "callee_param": param_name,  # passed positionally
                                    "callee_return": f"{callee_name}() -> return",
                                    "assign_to": lhs_name,
                                }
                            )

        # Return: return <expr using a tainted name> (direct or via alias)
        elif isinstance(stmt, ast.Return) and stmt.value is not None:
            ret_names = {n.id for n in ast.walk(stmt.value) if isinstance(n, ast.Name)}
            if ret_names & tainted:
                steps.append(
                    {
                        "source_ref": f"param:{param_name}",
                        "target_ref": "return",
        ...
        # Expr (bare call): f(tainted) without capturing the result.
...
def extract_data_flow_spines(src_path: Path, all_nodes: list[Node]) -> list[dict]:
    """Extract intraprocedural data flow spines for all module functions.

    For each function in each module node, traces how each non-self parameter
    flows through the function body to the return value.  Also records one-call-deep
    interprocedural flow when a parameter is passed to a known callee whose
    return value is assigned to a local variable.

    The result is stored as ``node['data_flow_spines']`` (a list of spine dicts)
    on each module node that has at least one traceable spine.  The function also
    returns the complete flat list for callers that want a single collection.

    This analysis is bounded per the spec:
    - Intraprocedural: each function body analysed independently.
    - Interprocedural: at most one call level deep.
    - No whole-program fixed-point analysis is performed.
    ...
    Args:
        src_path:  Root of the Python codebase.
        all_nodes: All nodes in the scene graph (module nodes are annotated
                   in-place with ``data_flow_spines``).

    Returns:
        A flat list of all spine dicts across all modules.
    """
    # Build function→module map for interprocedural resolution (one level deep).
    fn_to_module = _collect_function_names(src_path, all_nodes)

    all_spines: list[dict] = []
    ...
    for node in all_nodes:
        ...
        for py_file in sorted(node_path.rglob("*.py")):
            ...
            for func_node in ast.walk(tree):
                ...
                # Collect non-self/cls parameters to trace.
                param_names = [
                    arg.arg
                    for arg in func_node.args.args
                    if arg.arg not in ("self", "cls")
                ]
                # Also include positional-only args (Python 3.8+).
                param_names += [
                    arg.arg
                ...
                for param_name in param_names:
                    spine = _trace_parameter_spine(
                        func_node, param_name, fn_to_module, source_id
                    )
                    # Only emit spines that have meaningful flow (more than just
                    # the entry param_use step).
                    if len(spine["steps"]) > 1 or spine["interprocedural"]:
                        spine_entry: dict = {
                            "module_id": source_id,
                            "function_name": spine["function_name"],
                            "parameter": spine["parameter"],
                            "steps": spine["steps"],
                            "interprocedural": spine["interprocedural"],
                        }
                        node_spines.append(spine_entry)
                        all_spines.append(spine_entry)

        if node_spines:
            node["data_flow_spines"] = node_spines
...
def build_scene_graph(src_path: Path) -> SceneGraph:
    ...
    # 13. Extract data flow spines (intraprocedural + one-call-deep interprocedural).
    #     Annotates module nodes with data_flow_spines list.
    #     Spec: visual-primitives.spec.md §Data Flow Spine Extraction.
    extract_data_flow_spines(src_path, nodes)
